...
class BookForm(FlaskForm):
    ...
    director = StringField("Author", validators=[InputRequired()])

    year = IntegerField("Year", validators=[InputRequired(), NumberRange(min=1200, message="Please enter a year in the format YYYY")])
...
class ExtendedBookForm(BookForm):
    cast = StringListField("Characters")
    series = StringListField("Related Novels")
    tags = StringListField("Tags")
    description = TextAreaField("Description")
    video_link = URLField("Review Link")
